...
export class Content<T extends ContentStyle = 'Text Display'> extends Builder<ContentJson<T>> {
  ...
  constructor(data: ContentData<T>, style: T = 'Text Display' as T) {
    switch (style) {
      case 'Thumbnail':
        super({ type: 11, media: typeof data === 'string' ? { url: data } : data } as ContentJson<T>)
        break
      case 'Media Gallery': {
        const items = (Array.isArray(data) ? data : [data]) as (string | APIMediaGalleryComponent['items'][number])[]
        super({
          type: 12,
          items: items.map(item => (typeof item === 'string' ? { media: { url: item } } : item)),
        } as ContentJson<T>)
        break
      }
      case 'File':
        super({ type: 13, file: typeof data === 'string' ? { url: data } : data } as ContentJson<T>)
        break
